...
/**
 * AndroidResourceAnnotatedTypeFactory build types with <code>@XXXRes</code> or <code>@XXXContainer
 * </code> annotations.
 */
public class AndroidResourceAnnotatedTypeFactory extends BaseAnnotatedTypeFactory {
    ...
    protected final AnnotationMirror ANY_RES = AnnotationBuilder.fromClass(elements, AnyRes.class);
    ...
    protected final AnnotationMirror ID_RES = AnnotationBuilder.fromClass(elements, IdRes.class);
    ...
    protected final AnnotationMirror RAW_RES = AnnotationBuilder.fromClass(elements, RawRes.class);
    ...
    protected final AnnotationMirror XML_RES = AnnotationBuilder.fromClass(elements, XmlRes.class);
    ...
    private static final List<String> ANDROID_PACKAGE_PREFIXES =
            Arrays.asList(
    ...
    /**
     * Make the Android <code>@XXXRes</code> annotations aliases to <i>android-resource-checker</i>
     * annotations.
     */
    private void makeAndroidAnnotationsAliasesToChecker() {
        ANDROID_PACKAGE_PREFIXES.forEach(
                (packagePrefix) ->
                        ANNOTATION_MIRROR_MAP.forEach(
                                (annotationName, annotation) ->
                                        addAliasedAnnotation(
                                                packagePrefix + annotationName, annotation)));
    }
    ...
    private class AndroidResourceTreeAnnotator extends TreeAnnotator {
        ...
        /**
         * Assign <code>@XXXRes</code> annotations to member select. First it checks whether the
         * expression has the <code>@XXXContainer</code> annotation then allocates the corresponding
         * <code>@XXXRes</code> annotation.
         */
        private void assignResAnnotations(
                AnnotatedTypeMirror annotatedTypeMirror, ExpressionTree expressionTree) {
            ...
            AnnotationMirror expressionContainerAnnotation =
                    expressionAnnotatedTypeMirror.getAnnotationInHierarchy(RESOURCE_TOP);

            // checks if {expressionContainerAnnotation} is not null.
            if (expressionContainerAnnotation == null) return;
        ...
        private void assignContainerAnnotations(
                AnnotatedTypeMirror annotatedTypeMirror, MemberSelectTree node) {

            String expression = node.getExpression().toString();
            String identifier = node.getIdentifier().toString();

            // checks if expression doesn't equal to {R} -> return.
            if (!expression.equals("R")) return;
