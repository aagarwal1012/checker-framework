...
public class AndroidResourceVisitor extends BaseTypeVisitor<AndroidResourceAnnotatedTypeFactory> {
    ...
    protected void giveBinaryOperationWarning(BinaryTree node, Kind kind) {
        ...
        AnnotatedTypeMirror rightOperandType =
                atypeFactory.getAnnotatedType(node.getRightOperand());
        ...
        if (leftOperandHasResOrContainerAnnotation && !rightOperandHasResOrContainerAnnotation)
            checker.report(
                    Result.warning(
                            "binary.operation.left.operand.found.resource", kind, leftOperandType),
                    node);
    ...
    /**
     * Gives a warning whenever expression/variable in a compound assignment have a annotation of
     * type <code>@XXXRes</code> or <code>@XXXContainer</code>.
     */
    @Override
    public Void visitCompoundAssignment(CompoundAssignmentTree node, Void p) {
        ...
        if (expressionTypeHasResOrContainerAnnotations && !variableTypeHasResOrContainerAnnotations)
            checker.report(
                    Result.warning(
                            "compound.assignment.expression.found.resource", kind, expressionType),
                    node);
        else if (!expressionTypeHasResOrContainerAnnotations
                && variableTypeHasResOrContainerAnnotations)
            checker.report(
                    Result.warning(
                            "compound.assignment.variable.found.resource", kind, variableType),
                    node);
        else if (expressionTypeHasResOrContainerAnnotations
                && variableTypeHasResOrContainerAnnotations)
            checker.report(
    ...
    public boolean checkAnnotatedTypeHasResOrContainerAnnotations(
        ...
        AnnotationMirror annotationMirror =
                annotatedTypeMirror.getAnnotationInHierarchy(RESOURCE_TOP);
